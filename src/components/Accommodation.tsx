...
const roomAmenityIcons = [
  ...
  <svg key="ac" className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
    <path fillRule="evenodd" d="M11.3 1.046A1 1 0 0112 2v5h4a1 1 0 01.82 1.573l-7 10A1 1 0 018 18v-5H4a1 1 0 01-.82-1.573l7-10a1 1 0 011.12-.38z" clipRule="evenodd" />
  </svg>,
  <svg key="bath" className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
    <path fillRule="evenodd" d="M5.5 16a3.5 3.5 0 01-.369-6.98 4 4 0 117.753-1.977A4.5 4.5 0 1113.5 16h-8z" clipRule="evenodd" />
  </svg>,
  <svg key="wifi" className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
    <path fillRule="evenodd" d="M2 5a2 2 0 012-2h12a2 2 0 012 2v2a2 2 0 01-2 2H4a2 2 0 01-2-2V5zm14 1a1 1 0 11-2 0 1 1 0 012 0zM2 13a2 2 0 012-2h12a2 2 0 012 2v2a2 2 0 01-2 2H4a2 2 0 01-2-2v-2zm14 1a1 1 0 11-2 0 1 1 0 012 0z" clipRule="evenodd" />
  </svg>,
  <svg key="clean" className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
    <path d="M9 2a1 1 0 000 2h2a1 1 0 100-2H9z" />
    <path fillRule="evenodd" d="M4 5a2 2 0 012-2 3 3 0 003 3h2a3 3 0 003-3 2 2 0 012 2v11a2 2 0 01-2 2H6a2 2 0 01-2-2V5zm3 4a1 1 0 000 2h.01a1 1 0 100-2H7zm3 0a1 1 0 000 2h3a1 1 0 100-2h-3zm-3 4a1 1 0 100 2h.01a1 1 0 100-2H7zm3 0a1 1 0 100 2h3a1 1 0 100-2h-3z" clipRule="evenodd" />
  </svg>,
  <svg key="view" className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
...
export default function Accommodation() {
  ...
  const amenityLabels = [ac.beds, ac.ac, ac.bathroom, ac.wifi, ac.cleaning, ac.view];
    ...
    <section id="accommodation" className="section-padding bg-stone-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div ref={ref} className="text-center max-w-2xl mx-auto mb-12 md:mb-16">
          ...
          >
            {ac.title1}{' '}
            <span className="gradient-text">{ac.title2}</span>
          </motion.h2>
          <motion.p
            initial={{ opacity: 0, y: 20 }}
            animate={isInView ? { opacity: 1, y: 0 } : {}}
        ...
        >
          <div className="grid lg:grid-cols-2">
            ...
            <div className="relative min-h-[300px] lg:min-h-[520px] overflow-hidden">
              ...
              {/* Bottom badges */}
              <div className="absolute bottom-6 left-6 right-6 flex gap-2 sm:gap-3">
                {[
                  { label: ac.checkin, value: '14:00' },
                  { label: ac.checkout, value: '11:00' },
                  { label: ac.rating, value: '4.5 ★' },
                ].map((item) => (
                  <div key={item.label} className="bg-white/20 backdrop-blur-sm border border-white/30 rounded-xl px-2 py-2 flex-1 text-center">
                    <p className="font-inter text-white text-xs font-semibold leading-tight">{item.label}</p>
                    <p className="font-playfair text-white font-bold text-sm sm:text-base">{item.value}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="p-7 lg:p-12 flex flex-col justify-center">
              <div className="inline-flex items-center gap-2 bg-primary-50 text-primary-700 text-xs font-inter font-semibold px-3 py-1.5 rounded-full mb-6 self-start">
                <span>⭐</span>
                <span>{ac.popular}</span>
              </div>

              <h3 className="font-playfair text-2xl sm:text-3xl font-bold text-gray-900 mb-4">
                {ac.roomName}
              </h3>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2.5 mb-8">
                {amenityLabels.map((label, i) => (
                  <div
                    key={label}
                    className="flex items-center gap-2 bg-stone-50 rounded-xl p-3 border border-stone-200"
                  >
                    <div className="flex-shrink-0 text-primary-600">{roomAmenityIcons[i]}</div>
                    <span className="font-inter text-sm text-gray-700 font-medium leading-tight">{label}</span>
                  </div>
                ))}
              </div>
              ...
              <p className="font-inter text-xs text-gray-400 mt-4">{ac.noFees}</p>
            </div>
          </div>
        </motion.div>
      </div>
