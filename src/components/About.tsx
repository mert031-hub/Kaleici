            ...
            <div className="relative">
              <div className="relative rounded-3xl overflow-hidden shadow-soft-xl aspect-[4/5] max-w-md mx-auto lg:mx-0">
                ...
                <div className="absolute bottom-6 left-6 right-6 z-10 bg-white/20 backdrop-blur-sm border border-white/30 rounded-2xl p-5">
                  <p className="font-playfair text-xl font-semibold text-white mb-1">{ab.since}</p>
                  <p className="font-inter text-sm text-white/80">{ab.sinceDesc}</p>
                  <div className="flex items-center gap-3 mt-3">
                    <div className="flex -space-x-2">
                      {[1, 2, 3].map((i) => (
                        <div key={i} className="w-8 h-8 rounded-full border-2 border-white/40 bg-white/20" />
                      ))}
                    </div>
                    <span className="font-inter text-sm text-white/80">{ab.guests}</span>
                  </div>
                </div>
              </div>
            ...
            <AnimatedSection delay={0.15}>
              <div>
                ...
                <h2 className="heading-lg text-gray-900 mb-6">
                  {ab.title1}{' '}
                  <span className="gradient-text">{ab.title2}</span>
                </h2>
                <p className="font-inter text-base text-gray-600 leading-relaxed mb-4">{ab.desc1}</p>
                <p className="font-inter text-base text-gray-600 leading-relaxed">{ab.desc2}</p>
              </div>
            </AnimatedSection>
            ...
            <AnimatedSection delay={0.25}>
              <div className="grid grid-cols-3 gap-4">
                {[
                  ...
                  <div key={stat.label} className="text-center bg-white rounded-2xl p-4 shadow-soft border border-stone-200">
                    <p className="font-playfair text-2xl font-bold text-primary-700">{stat.number}</p>
